...
// Portal options presented after login (since no auth implemented yet)
const portals = [
  {
    id: 'employee',
    label: 'Employee Portal',
    desc: 'AI Assistant, IT Support, Password Reset',
    path: '/assistant',
    accent: 'var(--color-soc-accent)',
    accentGlow: 'var(--color-soc-accent-glow)',
    border: 'rgba(0,212,255,0.3)',
  },
  {
    id: 'admin',
    label: 'SOC / Admin Dashboard',
    desc: 'Incident Management, Threat Intelligence, Analytics',
    path: '/dashboard',
    accent: 'var(--color-soc-purple)',
    accentGlow: 'var(--color-soc-purple-glow)',
...
export default function LoginPage() {
  ...
  const [step, setStep]                 = useState('login')   // 'login' | 'select-portal'
  ...
  function handleSubmit(e) {
    e.preventDefault()
    if (validate()) setStep('select-portal')
  }
          ...
          {step === 'login' ? (
              ...
              <form onSubmit={handleSubmit} noValidate className="px-8 py-6 space-y-5">
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-xl bg-[var(--color-soc-accent)] text-[var(--color-soc-bg)] font-bold text-sm
                             hover:opacity-90 transition-opacity glow-accent flex items-center justify-center gap-2"
                >
            ...
            <div className="space-y-4 fade-in">
              <div className="text-center mb-8">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-[var(--color-soc-green-glow)] border border-[rgba(0,255,136,0.25)] mb-4">
                  <svg viewBox="0 0 24 24" fill="none" stroke="var(--color-soc-green)" strokeWidth={2.5} className="w-6 h-6">
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                </div>
                <h2 className="text-xl font-black text-[var(--color-soc-text)]">Authentication Successful</h2>
                <p className="text-sm text-[var(--color-soc-text-muted)] mt-1.5">Select your portal to continue</p>
              </div>
              ...
              {portals.map((portal) => (
                <button
                  ...
                  className="w-full text-left p-5 rounded-2xl bg-[var(--color-soc-card)] border border-[var(--color-soc-border-subtle)] hover:border-[rgba(0,212,255,0.3)] transition-all group hover:-translate-y-0.5"
                  style={{ '--hover-border': portal.border }}
                >
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                      style={{ background: portal.accentGlow, border: `1px solid ${portal.border}` }}>
                      <ShieldIcon cls="w-6 h-6" style={{ color: portal.accent }} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-[var(--color-soc-text)] text-sm">{portal.label}</p>
                      <p className="text-[11px] text-[var(--color-soc-text-muted)] mt-0.5">{portal.desc}</p>
                    </div>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5 text-[var(--color-soc-text-dim)] group-hover:text-[var(--color-soc-accent)] transition-colors flex-shrink-0">
              ...
              <button
                ...
                className="w-full py-2.5 text-xs text-[var(--color-soc-text-muted)] hover:text-[var(--color-soc-text)] transition-colors"
              >
